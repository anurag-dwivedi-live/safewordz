 ...
 * 
 * A toggle button component that handles theme switching between light and dark modes.
 * Supports system preference detection and user override persistence.
 * 
 * Features:
...
const ThemeButton = () => {
    ...
    const [isOverridden, setIsOverridden] = useState(() => saved === 'light' || saved === 'dark');
    
    // Initialize theme based on user preference or system setting
    const [theme, setTheme] = useState(() => {
        if (isOverridden) return saved;
        return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    });

    // Apply dark class to root element when theme changes
    useEffect(() => {
    ...
    useEffect(() => {
        if (isOverridden) return; // Skip if user has made a choice
    ...
    // Handle theme toggle and persist user choice
    const toggleTheme = () => {
    ...
    return (
        <label className="relative inline-flex cursor-pointer select-none items-center">
            ...
            />
            <div className="shadow-card flex h-[46px] w-[82px] items-center justify-center rounded-md bg-[#e2e8f0] dark:bg-[#334155]">
                {/* Light mode icon */}
                <span
                    className={`flex h-9 w-9 items-center justify-center rounded text-[#1e293b] dark:text-[#f8fafc] ${theme === 'light' ? 'bg-white dark:bg-[#1e293b]' : ''}`}
                >
                    <svg
                        width='16'
                        height='16'
                ...
                {/* Dark mode icon */}
                <span
                    className={`flex h-9 w-9 items-center justify-center rounded text-[#1e293b] dark:text-[#f8fafc] ${theme === 'dark' ? 'bg-white dark:bg-[#1e293b]' : ''}`}
                >
                    <svg
                        width='16'
                        height='16'
